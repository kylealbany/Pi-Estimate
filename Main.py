...
windowSize=800 # length and width in pixels of the window
lineWidth=5 # thickness of the drawn circle and square
...
while trials<maxTrials:
    ...
    if hypot(x-windowSize/2,y-windowSize/2)<=squareWidth/2:
        successes+=1
        pygame.draw.circle(window, (0,255,0), (int(x),int(y)),2, 0)
        pygame.display.update()
    # isn't inside circle
    else:
        pygame.draw.circle(window, (255,0,0), (int(x),int(y)),2, 0)
        pygame.display.update()
